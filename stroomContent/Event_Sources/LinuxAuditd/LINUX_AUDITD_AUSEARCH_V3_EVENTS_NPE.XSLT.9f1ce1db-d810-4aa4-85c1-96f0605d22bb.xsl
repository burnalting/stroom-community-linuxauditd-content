<xsl:stylesheet xmlns="event-logging:3" xpath-default-namespace="event-logging:3" xmlns:stroom="stroom" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="3.0">
  ...
  The purpose of this translation is to only translate those Linux auditd events where all user related elements are Non Person Entities
  such as root, unset, bin, etc. These users are identified by the {RunAs,User}/Type element containing the text 'NPE'.
  ...
  <xsl:template match="Events">
    <xsl:element name="Events">
      ...
      Thus if we are after NPE user based events then we select those that match
      (count(.//User) eq count(.//User/Type[text()='NPE'])) and (count(.//RunAs) eq count(.//RunAs/Type[text()='NPE']))
      -->
      <xsl:apply-templates select="Event[(count(.//User) eq count(.//User/Type[text()='NPE'])) and (count(.//RunAs) eq count(.//RunAs/Type[text()='NPE']))]" mode="nnpe" />
    </xsl:element>
  </xsl:template>

  <!-- Ensure at least one non-npe user -->
  <xsl:template match="Event" mode="nnpe">
